...
type LedgerItem = {
  ...
  debit: number;   // derived on backend
  credit: number;  // derived on backend
  running_balance: number; // derived on backend
};
...
export default function CustomerLedgerPage() {
  ...
  const decodedName = customerNameParam ? decodeURIComponent(customerNameParam) : undefined;
  ...
  const fetchLedger = useCallback(async () => {
    ...
    try {
      ...
      if (fromDate) params.from_date = new Date(fromDate + 'T00:00:00Z').toISOString();
      if (toDate) params.to_date = new Date(toDate + 'T23:59:59Z').toISOString();

      const res = await api.get<CustomerLedgerResponse>('/diiwaanpayments/search/by-customer-name', {
        headers: authHeader,
        params,
      });
      setLedger(res.data);
  ...
  }, [token, decodedName, authHeader, fromDate, toDate]);
  ...
  const sellerContact =
    profile?.phone_number?.trim() ? profile.phone_number!.trim() : (profile?.email?.trim() ?? '');
  ...
  const filteredItems: LedgerItem[] = useMemo(() => {
    ...
    return ledger.items.filter((row) => {
      ...
      const inInvoice = row.invoice_id != null && String(row.invoice_id).includes(q);
  ...
  // keep PDF/CSV description behavior unchanged
  const buildDescForPdf = (it: LedgerItem) => {
  const parts = [
    descOf(it),                                // 'Oil sale payment' fallback
  ...
  const buildLedgerHtml = (custName: string, items: LedgerItem[], totals: LedgerTotals) => {
    const rows = items
      .map(
        (it) => `
      <tr>
    ...
    return `
      ...
      <body>
        ...
        <div class="customer">Customer: ${escapeHtml(custName)}${range ? ` • Range: ${escapeHtml(range)}` : ''}</div>
        <table>
          <thead>
            <tr>
          ...
          <tbody>
            ${rows || `<tr><td colspan="5" style="text-align:center; padding:12px; color:#6B7280">No entries</td></tr>`}
          </tbody>
        </table>
        <div class="totals">
          <div class="card">
  ...
  const exportPdf = async () => {
    ...
    try {
      setExportBusy(true);
      const html = buildLedgerHtml(ledger.customer_name, filteredItems, ledger.totals);
      const { uri } = await Print.printToFileAsync({ html });
      await Sharing.shareAsync(uri, { mimeType: 'application/pdf', dialogTitle: 'Share Ledger PDF' });
    } catch (e: any) {
  ...
  const exportCsv = async () => {
    ...
    try {
      ...
      await Sharing.shareAsync(path, { mimeType: 'text/csv', dialogTitle: 'Share Ledger CSV' });
    } catch (e: any) {
  ...
  const shareWhatsAppPdf = async () => {
    ...
    try {
      ...
      if (digits) {
        const msg = `Ledger • ${ledger.customer_name}\nBalance: ${fmtMoney(ledger.totals.closing_balance)}${
          fromDate || toDate ? `\nRange: ${[fromDate, toDate].filter(Boolean).join(' → ')}` : ''
        }`;
        const link = `whatsapp://send?phone=${digits}&text=${encodeURIComponent(msg)}`;
  ...
  /* ----------------------------- UI ----------------------------- */
  const headerRight = (
    <TouchableOpacity onPress={() => !exportBusy && setExportOpen(true)} style={styles.headerAction} activeOpacity={0.9}>
      <Feather name="download-cloud" size={16} color={BRAND_BLUE} />
      <Text style={styles.headerActionTxt}>Export</Text>
    </TouchableOpacity>
  );

  // row renderer — NOTE-ONLY description + small date under it, no separate date column
  const renderItem = ({ item }: { item: LedgerItem }) => {
    const description = descOf(item);
    const isDebit = item.transaction_type === 'debit';
    const noteOnly = (item.note || '').trim();

    return (
      <View style={styles.row}>
        {/* Description (note only) + small date under it */}
        ...
        <View style={styles.colMoney}>
          <Text style={[styles.moneyTxt, isDebit ? styles.debit : styles.faint]} numberOfLines={1}>
            {isDebit ? fmtMoney(item.debit || 0) : '—'}
          </Text>
        </View>
        <View style={styles.colMoney}>
          <Text style={[styles.moneyTxt, !isDebit ? styles.credit : styles.faint]} numberOfLines={1}>
            {!isDebit ? fmtMoney(item.credit || 0) : '—'}
          </Text>
        </View>
        <View style={styles.colMoney}>
          <Text style={[styles.moneyTxt, styles.balance]} numberOfLines={1}>{fmtMoney(item.running_balance || 0)}</Text>
        </View>
      </View>
    );
  };
  ...
  return (
    <SafeAreaView style={styles.screen} edges={['top', 'left', 'right']}>
      ...
      <LinearGradient colors={[BRAND_BLUE, BRAND_BLUE_2]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.header}>
        ...
        {/* Quick KPIs (labels black for visibility) */}
        <View style={styles.kpis}>
          <View style={styles.kpiCard}>
            <Text style={[styles.kpiLabel, { color: '#111827' }]}>Total Debit</Text>
            <Text style={[styles.kpiValue, { color: AMBER }]}>{fmtMoney(ledger?.totals.total_debit)}</Text>
          </View>
          <View style={styles.kpiCard}>
            <Text style={[styles.kpiLabel, { color: '#111827' }]}>Total Credit</Text>
            <Text style={[styles.kpiValue, { color: SUCCESS }]}>{fmtMoney(ledger?.totals.total_credit)}</Text>
          </View>
          <View style={styles.kpiCard}>
            <Text style={[styles.kpiLabel, { color: '#111827' }]}>Balance</Text>
            <Text style={[styles.kpiValue, { color: DANGER }]}>{fmtMoney(ledger?.totals.closing_balance)}</Text>
          </View>
        </View>
      </LinearGradient>

      {/* Filters with popup date pickers + reset */}
      <View style={styles.filters}>
        ...
        >
          ...
          <Text style={[styles.dateDisplay, { color: fromDate ? TEXT : '#9CA3AF' }]}>
        ...
        >
          <Feather name="calendar" size={14} color={MUTED} />
          <Text style={[styles.dateDisplay, { color: toDate ? TEXT : '#9CA3AF' }]}>
      ...
      {/* Table header — DATE REMOVED */}
      <View style={styles.tableHeader}>
        <Text style={[styles.th, { flex: 3.4 }]}>Description</Text>
        <Text style={[styles.th, styles.thRight, { flex: 1.2 }]}>Debit</Text>
        <Text style={[styles.th, styles.thRight, { flex: 1.2 }]}>Credit</Text>
        <Text style={[styles.th, styles.thRight, { flex: 1.3 }]}>Balance</Text>
      </View>
      ...
      {loading ? (
        ...
        <View style={styles.center}>
          ...
          <TouchableOpacity onPress={fetchLedger} style={styles.retryBtn}><Text style={styles.retryTxt}>Try Again</Text></TouchableOpacity>
        </View>
      ) : (
        <FlatList
          data={filteredItems}
          ...
          ListEmptyComponent={<View style={styles.center}><Text style={styles.emptyText}>No entries in this range.</Text></View>}
          contentContainerStyle={{ paddingBottom: Math.max(20, (insets.bottom || 0) + 20) }}
      ...
      <Modal visible={exportOpen} transparent animationType="fade" onRequestClose={() => setExportOpen(false)}>
        <TouchableWithoutFeedback onPress={() => !exportBusy && setExportOpen(false)}>
          <View style={styles.backdrop} />
        </TouchableWithoutFeedback>
        <View style={[styles.centerModal, { paddingBottom: Math.max(16, insets.bottom + 8) }]}>
          <View style={styles.modalCard}>
            <Text style={styles.modalTitle}>Export / Send</Text>
            <Text style={styles.modalSub}>Choose a format to share the customer ledger.</Text>

            <TouchableOpacity style={[styles.actionRow, exportBusy && styles.actionRowDisabled]} onPress={exportPdf} disabled={exportBusy}>
              <View style={[styles.iconChip, { backgroundColor: '#EEF2FF' }]}><Feather name="file-text" size={16} color={BRAND_BLUE} /></View>
              <Text style={styles.actionTxt}>Download / Share PDF</Text>
              {exportBusy && exportGuard.current === 'pdf' ? <ActivityIndicator size="small" color={BRAND_BLUE} /> : <Feather name="chevron-right" size={18} color={MUTED} />}
            </TouchableOpacity>

            <TouchableOpacity style={[styles.actionRow, exportBusy && styles.actionRowDisabled]} onPress={exportCsv} disabled={exportBusy}>
              <View style={[styles.iconChip, { backgroundColor: '#ECFDF5' }]}><Feather name="grid" size={16} color={SUCCESS} /></View>
              <Text style={styles.actionTxt}>Download CSV (Excel)</Text>
              {exportBusy && exportGuard.current === 'csv' ? <ActivityIndicator size="small" color={SUCCESS} /> : <Feather name="chevron-right" size={18} color={MUTED} />}
            </TouchableOpacity>

            <TouchableOpacity style={[styles.actionRow, exportBusy && styles.actionRowDisabled]} onPress={shareWhatsAppPdf} disabled={exportBusy}>
              <View style={[styles.iconChip, { backgroundColor: '#E7F9EF' }]}><FontAwesome name="whatsapp" size={16} color="#25D366" /></View>
              <Text style={styles.actionTxt}>Send PDF via WhatsApp</Text>
              {exportBusy && exportGuard.current === 'wa' ? <ActivityIndicator size="small" color="#25D366" /> : <Feather name="chevron-right" size={18} color={MUTED} />}
            </TouchableOpacity>

            <TouchableOpacity style={[styles.modalCancel, exportBusy && { opacity: 0.6 }]} onPress={() => !exportBusy && setExportOpen(false)} disabled={exportBusy}>
...
const styles = StyleSheet.create({
  ...
  backBtn: {
    width: 32, height: 32, borderRadius: 8, borderWidth: 1, borderColor: 'rgba(255,255,255,0.25)',
    alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(255,255,255,0.08)',
  },
  headerTitle: { flex: 1, textAlign: 'center', color: '#fff', fontSize: 16, fontWeight: '900' },
  headerAction: {
    flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 10, height: 32,
    borderRadius: 10, backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: 'rgba(255,255,255,0.25)',
  },
  headerActionTxt: { color: BRAND_BLUE, fontWeight: '800', fontSize: 12 },

  kpis: { flexDirection: 'row', gap: 8, marginTop: 10, marginBottom: 6 },
  kpiCard: {
    flex: 1, backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: 'rgba(255,255,255,0.35)',
    borderRadius: 12, paddingVertical: 8, paddingHorizontal: 10,
  },
  kpiLabel: { fontSize: 11 },
  kpiValue: { color: '#FFFFFF', fontWeight: '900', fontSize: 14, marginTop: 2 },

  filters: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 14, marginTop: 10 },
  dateInputWrap: {
    flex: 1, flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#fff', paddingHorizontal: 10,
    borderRadius: 10, height: 40, borderWidth: 1, borderColor: '#DDE3F0',
  },
  dateDisplay: { flex: 1, fontSize: 13, paddingVertical: 2 },
  resetBtn: {
    flexDirection: 'row', alignItems: 'center', gap: 6, height: 40, paddingHorizontal: 12,
    borderRadius: 10, backgroundColor: '#EEF2FF', borderWidth: 1, borderColor: '#DDE3F0',
  },
  resetTxt: { color: BRAND_BLUE, fontWeight: '800', fontSize: 12 },

  searchWrapOuter: {
    marginTop: 8, marginBottom: 6, paddingHorizontal: 14, flexDirection: 'row', alignItems: 'center', gap: 8,
    backgroundColor: '#fff', borderRadius: 10, height: 40, borderWidth: 1, borderColor: '#DDE3F0',
  },
  searchInputOuter: { flex: 1, color: TEXT, fontSize: 14, paddingVertical: 2 },

  tableHeader: {
    flexDirection: 'row', paddingHorizontal: 14, paddingVertical: 8, backgroundColor: '#F5F7FB',
    borderTopWidth: 1, borderBottomWidth: 1, borderColor: '#EEF1F6',
  },
  th: { color: '#4B5563', fontSize: 12, fontWeight: '700' },
  thRight: { textAlign: 'right' as const },

  row: { flexDirection: 'row', paddingHorizontal: 14, paddingVertical: 10, backgroundColor: CARD },
  sep: { height: 1, backgroundColor: '#F0F2F5' },

  // columns (date column removed)
  colDesc: { flex: 3.4, paddingRight: 8, justifyContent: 'center' },
  colMoney: { flex: 1.2, alignItems: 'flex-end', justifyContent: 'center' },
  ...
  centerModal: {
    ...
    borderTopLeftRadius: 22, borderTopRightRadius: 22, paddingHorizontal: 16, paddingTop: 12,
  },
  modalCard: { gap: 10, paddingBottom: 12 },
  modalTitle: { fontSize: 16, fontWeight: '900', color: TEXT, textAlign: 'center' },
  modalSub: { fontSize: 12, color: MUTED, textAlign: 'center', marginBottom: 2 },
  actionRow: {
  ...
  actionTxt: { flex: 1, fontSize: 14, fontWeight: '800', color: '#0B1220' },
